...
docker run --name contrail-developer-sandbox -w /root -ti -v /var/run/docker.sock:/var/run/docker.sock -v contrail-dev-env-rpm-volume:/root/rpmbuild/RPMS -v $(pwd):/root/contrail-dev-env opencontrail/developer-sandbox:centos-7.4 || (docker start contrail-developer-sandbox && docker attach contrail-developer-sandbox)
